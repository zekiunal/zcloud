...
openssl req -new -x509 -days 3650 -extensions v3_ca -subj "/C=EU/ST=Izmir/L=Urla/O=Zetaops/O=ZetaopsCN=Zetaops Ulakbus CA" -key zato.ca.key.pem -out ca_cert.pem
echo ""
...
openssl req -new -key zato.load_balancer.key.pem -out zato.load_balancer.req.csr -subj "/C=EU/ST=Izmir/L=Urla/O=Zetaops/O=ZetaopsCN=Zetaops Ulakbus Load Balancer"
...
openssl req -new -key zato.web_admin.key.pem -out zato.web_admin.req.csr -subj "/C=EU/ST=Izmir/L=Urla/O=Zetaops/O=ZetaopsCN=Zetaops Ulakbus Web Admin"
...
openssl req -new -key zato.server$i.key.pem -out zato.server$i.req.csr -subj "/C=EU/ST=Izmir/L=Urla/O=Zetaops/O=ZetaopsCN=Zetaops Ulakbus Server $i"
